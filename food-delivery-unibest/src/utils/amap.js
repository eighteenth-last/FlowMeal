 ...
 * 添加自定义图标标记
 */
export function addIconMarker(map, lng, lat, iconHtml, opts = {}) {
  if (!map || !window.AMap) return null
  const content = `<div style="background:#fff;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.2);">${iconHtml}</div>`
  const marker = new window.AMap.Marker({
    position: new window.AMap.LngLat(lng, lat),
    content,
 ...
 * REST API
 */
export function drivingRoute(fromLng, fromLat, toLng, toLat) {
  return new Promise((resolve, reject) => {
    uni.request({
      url: `${REST_BASE}/direction/driving?key=${AMAP_KEY}&origin=${fromLng},${fromLat}&destination=${toLng},${toLat}&strategy=10`,
      success: (res) => {
